...
\textbf{2. Mixed Strategy Nash:} When no pure Nash exists, find mixed strategies using support enumeration. For a given support (subset of actions), compute mixed strategy probabilities using the indifference condition: a player is indifferent between all actions in the support, meaning they have equal expected payoff. This is implemented in the \texttt{\_solve\_for\_support} function which solves the system of indifference equations.
...
To comprehensively evaluate the three game-theoretic algorithms (Minimax, Nash Equilibrium, and Bayesian), we designed four distinct test scenarios with varying complexity levels. Each scenario was tested with 20 trials per algorithm, yielding a total of 240 trials (80 trials per algorithm).
