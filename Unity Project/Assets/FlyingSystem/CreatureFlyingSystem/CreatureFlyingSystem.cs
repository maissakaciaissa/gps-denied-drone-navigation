...
namespace FlyingSystem
{
    public class CreatureFlyingSystem : MonoBehaviour
    {
        ...
        public void Land()
        {
            ...
            creatureRigidbody.linearVelocity = Vector3.zero;
        ...
        void Fly()
        {
            if (inAir)
            {
                ...
                if (!inAirflow && canDive && meshRootTransform.localRotation.eulerAngles.x < 89.995f && meshRootTransform.localRotation.eulerAngles.x > divingStartAngle)
                {
                    ...
                    creatureRigidbody.linearVelocity = flyingVelocity + new Vector3(0.0f, Mathf.Clamp(verticalSpeed - verticalAcceleration * Time.deltaTime, -99999999.0f, 0.0f), 0.0f);
                }
                else
                {
                    if (!stopFlying)
                    {
                        ...
                        flyingSpeed = currentFlyingSpeed * staminaFactor * weightFactor;
                        flyingVelocity = flyingDirection * flyingSpeed;
                        creatureRigidbody.linearVelocity = flyingVelocity;
                        ...
                        if (airflowSpeed > 0.01f)
                        {
                            ...
                            creatureRigidbody.linearVelocity = flyingVelocity + Vector3.up * airflowSpeed;
                        }
                    }
                    else
                    {
