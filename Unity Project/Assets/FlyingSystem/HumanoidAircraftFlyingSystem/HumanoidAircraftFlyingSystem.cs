    ...
    public class HumanoidAircraftFlyingSystem : MonoBehaviour
    {
        ...
        void Fly()
        {
            ...
            if (Mathf.Abs(flyingDirection.x) > 0.01f || Mathf.Abs(flyingDirection.y) > 0.01f)
            {
                ...
                if (!hoverMode)
                    humanoidAircraftRigidbody.linearVelocity = new Vector3(flyingVelocity.x, humanoidAircraftRigidbody.linearVelocity.y, flyingVelocity.z);
                else
                    humanoidAircraftRigidbody.linearVelocity = flyingVelocity;
