...
class AlgorithmComparator:
    ...
    def visualize_comparison(self, save_path: Optional[str] = None, show: bool = True) -> None:
        ...
        bp1 = ax2.boxplot(path_data, labels=algorithms, patch_artist=True)
        ...
        bp2 = ax3.boxplot(battery_data, labels=algorithms, patch_artist=True)
        for patch, alg in zip(bp2['boxes'], algorithms):
            patch.set_facecolor(colors[alg])
        ax3.set_ylabel('Battery Used (%)', fontweight='bold')
        ...
        bp3 = ax4.boxplot(time_data, labels=algorithms, patch_artist=True)
        for patch, alg in zip(bp3['boxes'], algorithms):
            patch.set_facecolor(colors[alg])
        ax4.set_ylabel('Computation Time (seconds)', fontweight='bold')
